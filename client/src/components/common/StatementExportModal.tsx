...
export const StatementExportModal: React.FC<StatementModalProps> = ({
  ...
  const sha256Fingerprint = React.useMemo(() => {
    const raw = `${statementRef}-${displayUser?.email || 'user'}-${portfolio?.equity || 0}-${orders.length}`;
    let hash = 0;
    ...
    return `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852${hex1}${hex2}${hex3}${hex4}`.slice(0, 64);
  }, [statementRef, displayUser, portfolio, orders.length]);
