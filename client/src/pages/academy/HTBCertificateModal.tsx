...
import { X, FileDown, Award, Sparkles, CheckCircle2, Shield, User } from 'lucide-react';
...
export const HTBCertificateModal: React.FC<HTBCertificateModalProps> = ({
  ...
  const [studentName, setStudentName] = useState<string>(
    userName && userName !== 'Studente Quant'
      ? userName
      : localStorage.getItem('apex_lead_name') || ''
  );
  const [isGenerating, setIsGenerating] = useState(false);

  const effectiveName = studentName.trim() || 'Studente Quant';

  // Generate deterministic SHA-256 style hash for verification
  const verificationHash = `APX-QUANT-SHA256-${Array.from(effectiveName + completedDate)
  ...
  const handleDownload = () => {
    try {
      ...
      generateMasterclassCertificatePdf(effectiveName, completedDate, totalXp, verificationHash);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-amber-500/40 rounded-3xl w-full max-w-3xl flex flex-col shadow-[0_0_50px_rgba(245,158,11,0.15)] overflow-hidden font-mono">
        ...
        {/* Student Name Input for Personalization */}
        <div className="px-6 py-3 bg-slate-950/90 border-b border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-slate-300 w-full sm:w-auto">
            <User className="w-4 h-4 text-amber-400 shrink-0" />
            <span className="font-bold">Intestatario Diploma:</span>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-80">
            <input
              ...
              onChange={(e) => {
                ...
                setStudentName(val);
                if (val.trim()) {
                  localStorage.setItem('apex_lead_name', val.trim());
                }
              }}
              placeholder="Inserisci Nome e Cognome..."
              className="w-full px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-amber-300 placeholder-slate-500 text-xs font-bold font-mono focus:outline-none focus:ring-1 focus:ring-amber-400"
            />
          </div>
        </div>
