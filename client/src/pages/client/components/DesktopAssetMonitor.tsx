    ...
    <div className="bg-slate-900/90 rounded-2xl p-4 border border-slate-800 flex flex-col h-full space-y-3 font-mono text-xs shadow-xl">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-white placeholder:text-slate-600 focus:outline-none focus:border-cyan-500 text-xs min-h-[44px]"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-1 p-1 bg-slate-950 rounded-xl border border-slate-800/80 text-[10px]">
        {(['ALL', 'CRYPTO', 'FOREX', 'COMMODITY', 'INDEX', 'STOCK'] as const).map((cat) => (
          <button
            ...
            className={`px-2.5 py-1.5 rounded-lg font-bold transition-all cursor-pointer min-h-[36px] ${
              category === cat
          ...
          filtered.map((q) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  triggerHaptic('light');
                  onSelectSymbol(q.symbol);
                }}
                className={`w-full p-2.5 rounded-xl border flex items-center justify-between transition-all cursor-pointer text-left min-h-[48px] active:scale-[0.99] ${
                  isSelected
